...
def get_dataset(dataset_file):
    ...
    return ImageDataBunch.from_folder(dataset_file,
                                      valid_pct=0.2,
                                      size=IMAGE_SIZE,
                                      bs=bs,
...
def train_model(model_name, data_to_load, cycle):
    """Train a model from base available model"""
    model_to_train = load_deep_learning_model(get_dataset(data_to_load), model_name)

    print("Training Model from now")
    model_to_train.unfreeze()
    model_to_train.fit_one_cycle(cycle, max_lr=slice(1e-6, 1e-3))
...
train_model(RESNET_152, SOYBEAN_ROOT_PATH, 100)

def compute_sum(n, x):
    sum_total = 0
    for i in range(n - 2):
        i += 2
        sum_total += 100 * ((x[i] - x[i - 1] ** 2) ** 2 + (1 - x[i - 1] ** 2))
